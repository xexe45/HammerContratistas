<section class="filosofia text-center">
	<h2 class="titulo-f"> Más de 2 años <br>respaldan nuestro trabajo profesional </h2>
</section>
<section class="container-fluid">
		<div class="row">
			<div class="col-md-6 conocenos">
				<div class="row">
					<div class="col-md-12 conocer">
						<h2 class="texto2">CONÓCENOS</h2>
					</div>
				</div>
				<div class="row">
					<p class="parrafo">
						<?=$info->v2?>
					</p>
					
				</div>
				
				
			</div>
			<div class="col-md-6">
				<div id="carouselExampleSlidesOnly" class="carousel slide" data-ride="carousel">
				  <div class="carousel-inner">
				    <div class="carousel-item active">
				      <img class="d-block w-100" height="600" src="<?=base_url()?>assets/imgs/filosofia/<?=$info->v5?>" alt="First slide">
				    </div>
				    <div class="carousel-item">
				      <img class="d-block w-100" height="600" src="<?=base_url()?>assets/imgs/filosofia/<?=$info->v6?>" alt="Second slide">
				    </div>
				    
				  </div>
				</div>
			</div>
		</div>
</section>
<section class="info">
	<div class="container">
		<div class="row">
			<div class="col-md-4">
				<img class="img-fluid" src="<?=base_url()?>assets/imgs/filosofia/casquin.png" alt="">
			</div>
			<div class="col-md-4">
				<div class="card">
				  <div class="card-body">
				    <h3 class="vis">MISIÓN</h3>
				    <p class="vismis">
				    	<?=$info->v3?>
				    </p>
				  </div>
				</div>
			</div>
			<div class="col-md-4">
			<div class="card">
			  <div class="card-body">
			    <h3 class="vis">VISIÓN</h3>
			    <p class="vismis">
			    	<?=$info->v4?>
			    </p>
			  </div>
			</div>
			</div>
		</div>
		
	</div>
</section>
